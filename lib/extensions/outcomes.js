...
(function() {
  ...
  OutcomeDocument = (function() {
    function OutcomeDocument(type, source_did, outcome_service) {
      ...
      this.container = this.doc.ele('imsx_POXBody').ele(type + 'Request');
      this.body = this.container.ele('resultRecord');
    ...
    OutcomeDocument.prototype.add_submission_time = function(submission_time) {
      return this.container.ele('submissionDetails').ele('submittedAt', submission_time);
    };
  ...
  OutcomeService = (function() {
    ...
    OutcomeService.prototype.send_replace_result = function(score, submission_time, callback) {
      var doc, err;
    ...
    OutcomeService.prototype.send_replace_result_with_text = function(score, submission_time, text, callback) {
      var doc, err;
      doc = new OutcomeDocument(this.REQUEST_REPLACE, this.source_did, this);
      try {
        doc.add_score(score, this.language, doc.add_submission_time(submission_time, doc.add_text(text)));
        return this._send_request(doc, callback);
      } catch (error) {
        err = error;
        return callback(err, false);
    ...
    OutcomeService.prototype.send_replace_result_with_url = function(score, submission_time, url, callback) {
      var doc, err;
      doc = new OutcomeDocument(this.REQUEST_REPLACE, this.source_did, this);
      try {
        doc.add_score(score, this.language, doc.add_submission_time(submission_time, doc.add_url(url)));
        return this._send_request(doc, callback);
      } catch (error) {
        err = error;
        return callback(err, false);
    ...
    OutcomeService.prototype._process_response = function(body, callback) {
      return xml2js.parseString(body, {
        ...
        return function(err, result) {
          ...
          if (err) {
            return callback(new errors.OutcomeResponseError('Your LMS sent back an invalid response; please contact Perusall Support.'), false);
          }
